...
/**
 * Add a WordPress wp-admin admin menu for the plugin under the "BuddyPress" menu.
 */
function crumbs_add_admin_menu() {
	...
	add_submenu_page( 'bp-general-settings', __( 'Breadcrumbs Everywhere', 'breadcrumbs-everywhere' ), __( 'Breadcrumbs Everywhere', 'breadcrumbs-everywhere' ), 'manage_options', 'crumbs-settings', 'crumbs_admin' );
}
...
function crumbs_admin() {
	...
	if ( '' == $setting_one && option_value!= null ) { //true
					$setting_one =  __( 'Home', 'breadcrumbs-everywhere' );
					}
					
	$setting_two =  get_option('crumbs-setting-two');		
	if ( '' == $setting_two && option_value!= null ) { //true
				    $setting_two =  __( 'Blog', 'breadcrumbs-everywhere' );
					}
					
	$setting_three =  get_option('crumbs-setting-three');			
	if ( '' == $setting_three && option_value!= null ) { //true
					$setting_three =  __( '&raquo;', 'breadcrumbs-everywhere' );
					}
	
?>
<div class="wrap">
  <h2><?php _e( 'Breadcrumbs Everywhere', 'breadcrumbs-everywhere' ) ?></h2>

  <?php if ( isset( $updated ) ) : ?><?php echo '<div id="message" class="updated fade"><p>' . __( 'Settings Updated.', 'breadcrumbs-everywhere' ) . '</p></div>' ?><?php endif; ?>
  ...
  <table class="form-table">
    <tr valign="top">
      <th scope="row"><label for="target_uri"><?php _e( 'Text for the Home link:', 'breadcrumbs-everywhere' ) ?></label></th>
        <td><input name="crumbs-setting-one" type="text" id="crumbs-setting-one" value="<?php echo esc_attr( $setting_one ); ?>" size="60" /></td>
    </tr>
    <tr valign="top">
      <th scope="row"><label for="target_uri"><?php _e( 'Text for the Blog link:', 'breadcrumbs-everywhere' ) ?></label></th>
        <td><input name="crumbs-setting-two" type="text" id="crumbs-setting-two" value="<?php echo esc_attr( $setting_two ); ?>" size="60" /></td>
    </tr>
    <tr valign="top">
      <th scope="row"><label for="target_uri"><?php _e( 'Breadcrumb divider:', 'breadcrumbs-everywhere' ) ?></label></th>
  ...
  <p class="submit">
  <input type="submit" name="submit" value="<?php _e( 'Save Settings', 'breadcrumbs-everywhere' ) ?>"/>
  </p>
  <span><em><?php _e( "That's it!", "breadcrumbs-everywhere" ) ?></em></span>
